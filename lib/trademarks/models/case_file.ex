defmodule Trademarks.CaseFile do
  ...
  def process(stream) do
    ...
    number_of_trademarks_before = Trademarks.Repo.one(from cfo in "trademarks", select: count(cfo.id))
    ...
    number_of_trademarks_now = Trademarks.Repo.one(from cfo in "trademarks", select: count(cfo.id))
    ...
    Logger.info fn ->
      ...
      #{new_trademarks} trademarks and #{new_case_file_owners} new case file owners in #{finished - started} secs\n
      """
    end
  end
  ...
  defp associate_with_case_file(case_file_id, case_file_owner_id) do
    %CaseFilesCaseFileOwner{}
    |> CaseFilesCaseFileOwner.changeset(%{case_file_id: case_file_id,
  ...
  defp associate_with_trademark(trademark_id, case_file_owner_id) do
    %CaseFileOwnersTrademark{}
    |> CaseFileOwnersTrademark.changeset(%{trademark_id: trademark_id,
                                           case_file_owner_id: case_file_owner_id})
    |> Repo.insert(on_conflict: :nothing)
  end
